...
sub new {
    ...
    my $self     = $class -> SUPER::new("timefmt"      => '%d %B %Y',
                                        "timefmt_full" => '%d %B %Y',
                                        @_)
        or return undef;
...
sub generate_feed {
    ...
    foreach my $result (@{$results}) {
        ...
        if($settings -> {"images"}) {
            ...
            $images[1] = $self -> {"article"} -> {"images"} -> get_image_url($result -> {"images"} -> [1], 'ppcompat');
        }

        # Wrap the images in html
        $images[0] = $self -> {"template"} -> load_template("feeds/cssite/image.tem", {"***class***" => "leader",
                                                                                       "***url***"   => $images[0],
                                                                                       "***title***" => $result -> {"title"}})
            if($images[0]);

        $images[1] = $self -> {"template"} -> load_template("feeds/cssite/image.tem", {"***class***" => "article",
                                                                                       "***url***"   => $images[1],
                                                                                       "***title***" => $result -> {"title"}})
            if($images[1]);
        ...
        my $summary = $self -> {"template"} -> load_template("feeds/cssite/item-$mode-$showsum", {"***summary***" => $result -> {"summary"},
                                                                                                  "***link***"    => $feedurl,
                                                             });
        ...
        if($result -> {"files"} && scalar(@{$result -> {"files"}})) {
            foreach my $file (@{$result -> {"files"}}) {
                $files .= $self -> {"template"} -> load_template("feeds/cssite/file.tem", {"***name***" => $file -> {"name"},
                                                                                           "***size***" => $self -> {"template"} -> bytes_to_human($file -> {"size"}),
                                                                                           "***url***"  => $self -> {"article"} -> {"files"} -> get_file_url($file)});
            }

            $files = $self -> {"template"} -> load_template("feeds/cssite/files.tem", {"***files***" => $files})
                if($files);
        }


        # Put the item together!
        $items .= $self -> {"template"} -> load_template("feeds/cssite/item-$mode.tem", {"***title***"       => $result -> {"title"} || $pubdate,
                                                                                         "***summary***"     => $summary,
                                                                                         "***leaderimg***"   => $images[0],
                                                                                         "***articleimg***"  => $images[1],
    ...
    my $feed = $self -> {"template"} -> load_template("feeds/cssite/channel.tem", {"***title***"       => $self -> {"settings"} -> {"config"} -> {"RSS:title"},
