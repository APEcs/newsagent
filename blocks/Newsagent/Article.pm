...
##@method protected $ _build_feed_stickysupport($feeds)
# Generate a block of javascript that determines which feeds support sticky mode.
#
# @param feeds A hash generated by get_user_levels()
# @return A string containing the javascript to embed in the page
sub _build_feed_stickysupport {
    my $self  = shift;
    my $feeds = shift;
    ...
    foreach my $feed (@{$feeds}) {
        my $row = '"'.$feed -> {"id"}.'": '.($feed -> {"allowsticky"} ? "true" : "false");
        push(@incantation, $row);
    }
